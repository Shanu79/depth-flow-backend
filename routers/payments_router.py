...
def check_if_already_purchased(email: str, product_id: str) -> bool:
    ...
    """
    try:
        # NOTE: Check your Dodo SDK documentation for the exact filter syntax.
        # Most payment SDKs allow filtering by customer email or metadata.
        payments = client.payments.list(
            customer={"email": email}, 
            limit=100
        )
        
        # Iterate through history to find the product
        for payment in payments.items: # .items depends on SDK response structure
            if (payment.product_id == product_id and 
        ...
        # Fail safe: If we can't check, assume False (or True to be strict)
        return False
